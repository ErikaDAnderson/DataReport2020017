...
(ref:sharkphoto) Common thresher shark (*Alopias vulpinus*) captured during gear optimization of the offshore LFS 1142 trawl net (despite the open cod end). This 4.6 m shark was released alive (photo credit T. Zubkowski).

```{r sharkphoto, fig.pos="H", fig.cap = "(ref:sharkphoto)"}

# load image
knitr::include_graphics("data/threshershark.JPG")
...
(ref:forkck) Juvenile chinook salmon (*Oncorhynchus tshawytscha)*) length frequency plot as sampled during the `r surveyName` aboard the `r vessel`, `r surveyDateRange` (A). Double log-transformed length-weight regression with outliers removed, using a Bonferroni outlier test (B). No linear equation is presented since there were so few caught.
