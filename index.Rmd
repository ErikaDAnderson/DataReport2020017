...
events_orig <- readr::read_csv("data/csv/2020-017_BridgeLog_digital_2020-11-18.csv",
                                col_types = cols(
...
bridge <- events %>%
  ...
  mutate(Gear = str_c("LFS ", str_extract(Type, "[0-9]+"), " trawl net"),
         Test = str_extract(Comments, "DO NOT USE"),
         # from start-stop positions
         ...
         DISTANCE_M = round((mapply(lat_a =  Start_Lat_DD, lon_a = Start_Long_DD, #calc. dist.
                           lat_b = End_Lat_DD, lon_b = End_Long_DD,
                           FUN = calcDist)),2)) %>%
  select(Event, Date, Location, Type, StartTime, TowTime, Start_Lat_DD, Start_Long_DD, End_Lat_DD, End_Long_DD, HEADING, DISTANCE_NM, DISTANCE_M, Speed, VerticalNetOpening, Wingspread, Warp, TargetHeadDepth, ActualHeadDepth, Start_Bot_Depth, StopBotDepth, Comments, Gear, Test)

bridgetow <- bridge %>%
  filter(!(is.na(Gear))) %>%
  mutate(Usable = if_else(is.na(Test), "Y", "N"))
