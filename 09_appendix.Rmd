...
(ref:offshorenet1) Net specifications (upper view) for the offshore NPAFC 1142 mid water trawl net used on the the `r surveyName` from `r surveyDateRange` on the *`r vessel`*.
...
(ref:offshorenet2) Net specifications (lower view) for the offshore NPAFC 1142 mid water trawl net used on the the `r surveyName` from `r surveyDateRange` on the *`r vessel`*.
...
(ref:offshorenet3) Net specifications (side view) for the offshore NPAFC 1142 mid water trawl net used on the the `r surveyName` from `r surveyDateRange` on the *`r vessel`*.
...
(ref:coastalnet) Net specifications (side view only) for the coastal NPAFC 7742 mid water trawl net the `r surveyName` from `r surveyDateRange` on the *`r vessel`*.
...
(ref:oceantable) CTD casts and plankton tow times and depths during the `r surveyName` from `r surveyDateRange` on the *`r vessel`*.
...
csasdown::csas_table(catch2,
                     ...
                     format = "latex", landscape = TRUE,
                     repeat_header = TRUE, repeat_header_text = "continued",
                     extra_header = c("Tow", as.character(12:22)),
                     ex_align = "r", ex_line_sep = 0) %>%
  row_spec(21, hline_after = T)
